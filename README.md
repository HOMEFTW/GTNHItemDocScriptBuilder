...
当前导入 MVP 支持本工具生成的主要格式：GTNH RA2 builder、GT `RecipeRemover.remove(...)`、有序/镜像有序/无序合成、熔炉配方、熔炉燃料、熔炉删除和普通删除。复杂手写脚本会按脚本出现顺序解析第一条匹配的受支持配方，不会执行或完整理解所有 ZenScript 语法。

右侧 ZS 预览分为上下两半：上半区显示导入的完整 `.zs` 文件原文，下半区显示当前草稿会保存出来的内容。两个区域都有竖向和横向滚动条。导入脚本后，下半区标题会标出当前草稿来自哪一个文件、脚本中的第几条受支持配方、起始行号和调用类型；当前 MVP 仍只把第一条受支持配方载入草稿，完整多配方选择列表留到下一步。
...
GUI 的第一 UI 优先级是保持当前三栏比例：左侧物品搜索栏固定宽度，右侧 ZS 预览固定宽度，中间编辑区使用剩余空间。默认窗口为 `1500 x 920`，旧保存配置的高度低于 `900` 时会自动提升；后续新增控件时必须先保证这个比例不被破坏；左侧、中间和右侧长内容依靠各自滚动条查看，不允许撑宽侧栏挤压中间编辑区。
